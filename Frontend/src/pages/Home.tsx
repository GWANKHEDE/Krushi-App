...
import {
  ...
  ArrowRight, CheckCircle, Loader2, ShoppingBag, Phone,
} from "lucide-react"
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      if (el) {
        const next = (slide + 1) % heroImages.length
  ...
  return (
    <div className="min-h-screen bg-background font-sans">
      ...
      {/* ═══════════════════════════════════════════════
          HERO — iOS scroll-snap carousel with entrance
          ═══════════════════════════════════════════════ */}
      <section className="relative overflow-hidden" style={{ height: "88vh", minHeight: 480 }}>
        {/* Carousel track */}
        <div
          id="hero-carousel"
        ...
        >
          {heroImages.map((img, i) => (
            <div key={i} style={{ minWidth:"100%", height:"100%",
              ...
              {/* Parallax-like scale on the bg image */}
              <div style={{
              ...
              <div style={{ position:"absolute", inset:0,
                background:"linear-gradient(to right, rgba(0,0,0,.82) 0%, rgba(0,0,0,.40) 55%, rgba(0,0,0,.10) 100%)" }} />
            </div>
          ))}
        </div>
          ...
          <div className="max-w-2xl pointer-events-auto space-y-5">
            ...
            <div className="pub-hero-btns flex flex-col sm:flex-row gap-3">
              <Button asChild size="lg"
                className="bg-primary hover:bg-primary/90 text-white rounded-2xl h-12 px-7 text-sm font-bold shadow-xl shadow-primary/30 pub-btn-press">
                <Link to="/products">
                  {t("explore_products")}
              ...
              <Button asChild variant="outline" size="lg"
                className="bg-white/10 backdrop-blur-md border-white/30 text-white hover:bg-white/20 rounded-2xl h-12 px-7 text-sm font-bold pub-btn-press">
                <Link to="/contact">
          ...
          {heroImages.map((_, i) => (
            <button key={i}
              ...
              style={{
                ...
                background: i === slide ? "white" : "rgba(255,255,255,.42)",
        ...
        {[{ dir:-1, pos:"left-4", label:"‹" },{ dir:1, pos:"right-4", label:"›" }].map(({ dir, pos, label }) => (
          <button key={dir}
            ...
            className={`absolute top-1/2 -translate-y-1/2 ${pos} z-20 hidden md:flex items-center justify-center transition-all hover:scale-110 `}
            style={{ width:44, height:44, borderRadius:"50%",
              background:"rgba(255,255,255,.15)", backdropFilter:"blur(16px)",
              border:"0.5px solid rgba(255,255,255,.28)", color:"white",
      ...
      <section className="py-4 border-y border-border/50 bg-muted/30 overflow-hidden">
        <div className="pub-marquee gap-12 text-sm font-semibold text-muted-foreground select-none">
          {[...Array(2)].map((_, ri) => (
            <div key={ri} className="flex items-center gap-12 pr-12">
              {["🌾 5000+ Farmers Served","✅ 100% Genuine Products","📞 Expert Support","🏆 10+ Years Experience",
                "🚜 Marathwada's Trusted Store","🌱 Eco-Friendly Options","💰 Best Prices Guaranteed"].map((t, i) => (
      ...
      <section className="py-12 container mx-auto px-4">
        <FadeUp>
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary via-primary to-emerald-900 text-white shadow-2xl shadow-primary/20">
            {/* Decorative circles */}
            <div className="absolute -top-20 -right-20 w-80 h-80 rounded-full bg-white/8 pub-glow-pulse" />
            <div className="absolute -bottom-16 -left-16 w-64 h-64 rounded-full bg-accent/10 pub-float-slow" />
            <div className="absolute inset-0 opacity-5"
              style={{ backgroundImage:"radial-gradient(circle at 1.5px 1.5px,white 1.5px,transparent 0)", backgroundSize:"22px 22px" }} />
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 p-8 md:p-12 items-center">
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-4 items-center">
                  <Button asChild size="default"
                    className="bg-yellow-400 text-primary hover:bg-yellow-300 rounded-xl h-11 px-6 font-bold shadow-xl pub-btn-press">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <div className="flex gap-2 text-base font-black tabular-nums">
                      {["02d","14h","45m"].map(v => (
                        <span key={v} className="bg-black/25 px-2.5 py-1 rounded-lg">{v}</span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-center lg:justify-end">
                <img src={ureaImg} alt="Offer" className="h-48 md:h-56 object-contain drop-shadow-2xl pub-float" />
              </div>
            </div>
          </div>
        </FadeUp>
      ...
      <section className="py-14 bg-primary text-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-5"
          style={{ backgroundImage:"radial-gradient(circle at 2px 2px,white 1.5px,transparent 0)", backgroundSize:"26px 26px" }} />
            ...
            {categories.map((cat, i) => (
              <SlideIn key={cat.id} delay={i * 90} direction={i % 2 === 0 ? "left" : "right"}>
                <Link to={`/products?category=${cat.name}`} className="group block">
                  <div className="pub-img-zoom relative rounded-3xl overflow-hidden aspect-[4/5] bg-muted shadow-md group-hover:shadow-2xl transition-shadow duration-500">
                    <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent z-10" />
                    <img src={getCategoryImage(i)} alt={cat.name} className="h-full w-full object-cover" />
                    <div className="absolute bottom-0 left-0 right-0 p-6 z-20 translate-y-3 group-hover:translate-y-0 transition-transform duration-500">
                      <h3 className="text-xl font-black text-white uppercase tracking-tight mb-1">{cat.name}</h3>
                      <div className="h-0.5 w-0 group-hover:w-14 bg-primary rounded-full transition-all duration-500" />
                      <p className="text-white/65 text-xs font-bold uppercase tracking-widest mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <FadeUp className="text-center mb-14">
            <Badge className="bg-accent text-accent-foreground px-4 py-1 uppercase text-[10px] font-black tracking-widest border-none mb-3">
              Just In
            </Badge>
          ...
          <StaggerList className="grid grid-cols-1 md:grid-cols-2 gap-6" stagger={90}>
            {newArrivals.map((product) => (
              <div key={product.id}
                ...
                <Badge className="absolute top-3 left-3 z-10 bg-primary text-white text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg border-none">
                  New
                </Badge>
                ...
                <div className="w-2/3 pl-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button asChild size="sm" variant="ghost"
                      className="h-8 rounded-xl px-3 text-[11px] font-bold text-primary hover:bg-primary/10 transition-colors">
      ...
      <section className="py-20 bg-muted/10">
        <div className="container mx-auto px-4">
          <FadeUp className="flex flex-col md:flex-row justify-between items-center mb-14 gap-4">
            ...
            <Button asChild size="default"
              className="bg-primary text-white hover:bg-primary/90 rounded-full px-6 h-10 text-sm font-bold shadow-lg pub-btn-press">
                ...
                <div className="pub-card-hover group overflow-hidden rounded-[2rem] bg-card border border-border/50">
                  <div className="pub-img-zoom relative aspect-[4/3] bg-muted/30">
                    ...
                    <div className="absolute top-5 right-5 z-10">
                      <Badge className={`shadow px-3 py-0.5 rounded-full text-[10px] font-bold border-none ${
                        product.currentStock > 10 ? "bg-green-500 text-white" : "bg-destructive text-white"
                      }`}>
                  ...
                  <div className="p-7">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-base font-black text-foreground uppercase tracking-tight group-hover:text-primary transition-colors line-clamp-1 pub-underline-grow">{product.name}</h3>
                    ...
                    <div className="flex items-center justify-between pt-5 border-t border-border/50">
                      ...
                      <Button size="sm" asChild
                        className="bg-primary hover:bg-primary/90 text-white rounded-xl px-5 h-9 text-xs font-bold pub-btn-press ">
      ...
      <section className="py-24 bg-primary text-white relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 -ml-24 -mb-24 w-96 h-96 bg-accent/10 rounded-full blur-3xl pub-float" />
        <div className="absolute inset-0 opacity-5"
          style={{ backgroundImage:"radial-gradient(circle at 2px 2px,white 2px,transparent 0)", backgroundSize:"28px 28px" }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="space-y-7">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i}
                      className="flex items-start gap-5 bg-white/8 hover:bg-white/14 border border-white/8 hover:border-white/20 rounded-2xl p-5 group transition-all duration-300">
            ...
            <SlideIn direction="right">
              <div className="relative">
                ...
                <div className="relative rounded-[2.5rem] overflow-hidden border-4 border-white/12 shadow-2xl pub-img-zoom">
                  <img src="https://images.unsplash.com/photo-1625246333195-78d9c38ad449?q=80&w=800&auto=format&fit=crop"
                    alt="Farm" className="w-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
                </div>
                {/* Floating stat */}
                <ScaleIn delay={400}>
                  <div className="absolute -bottom-5 -left-5 bg-card text-foreground p-4 rounded-2xl shadow-2xl border border-border/50 hidden md:block pub-float">
      ...
      <section className="py-24 container mx-auto px-4">
        <FadeUp>
          <div className="relative overflow-hidden rounded-[3rem] bg-gradient-to-br from-primary via-primary to-emerald-900 text-white p-12 md:p-20 text-center shadow-2xl shadow-primary/20">
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button asChild size="lg"
                  className="bg-white text-primary hover:bg-white/90 rounded-full h-12 px-10 text-sm font-bold shadow-xl pub-btn-press">
                  <Link to="/products">{t("shop_now")} <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
                <Button asChild variant="outline" size="lg"
                  className="bg-white/10 border-white/30 text-white hover:bg-white/20 rounded-full h-12 px-10 text-sm font-bold backdrop-blur-sm pub-btn-press">
